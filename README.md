...
Сборка проекта осуществляется средствами [*CMake*](https://cmake.org/). Используйте встроенные средства среды разработки либо произведите сборку проекта вручную через терминал.

Для сборки проекта в терминале выполните следующие команды:
```shell
...
**Примечание**. Можете оставить здесь ссылку на _zip_-архив с тестовыми данными на [Google Drive](https://drive.google.com/drive/folders/1tjAq0AGUR0yBCoX4BvQQ-4NxBFtjU1nR?usp=sharing). Обратите внимание, что у вашего проекта должна быть собственная папка на Google Drive.

Тестовые данные представлены в CSV формате:
- каждая строка в файле формирует сущность (_entity_) с полями `id`, `full_name`, и пр.
